...
from backend.app.collectors.exceptions import ValidationError
...
class RawPayloadValidator:
    ...
    def validate_repository_payload(
        ...
        if not isinstance(raw_data, dict):
            raise ValidationError(f"Payload must be a dictionary, got {type(raw_data)}")

        # 1. Required field: name
        repo_name = raw_data.get("name")
        if not repo_name or not isinstance(repo_name, str) or not repo_name.strip():
            raise ValidationError("Payload missing required non-empty string field 'name'")

        # 2. Required field: owner
        owner_val = raw_data.get("owner")
        if owner_val is None:
            raise ValidationError("Payload missing required field 'owner'")
        if isinstance(owner_val, dict):
            login = owner_val.get("login")
            if not login or not isinstance(login, str) or not login.strip():
                raise ValidationError("Payload owner dictionary missing non-empty 'login' handle")
        elif not isinstance(owner_val, str) or not owner_val.strip():
            raise ValidationError("Payload owner field must be a valid dict or string login")

        # 3. Validate integer ID if present
        if "id" in raw_data:
            repo_id = raw_data["id"]
            if not isinstance(repo_id, int) or repo_id <= 0:
                raise ValidationError(f"Invalid repository ID '{repo_id}'; must be a positive integer")
