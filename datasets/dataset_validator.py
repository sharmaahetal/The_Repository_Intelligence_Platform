    ...
    def validate_dataset_rows(self, rows: list[dict[str, Any]]) -> None:
        ...
        for idx, row in enumerate(rows):
            ...
            if not full_name or not snapshot_time:
                raise ValueError(
                    f"Row #{idx} missing required provenance key 'full_name' or 'snapshot_time'"
                )
            ...
            if key in seen_keys:
                raise ValueError(
                    f"Duplicate (repository_id, snapshot_time) pair detected in dataset: {key}"
                )
            seen_keys.add(key)
            ...
            if isinstance(horizon_days, (int, float)) and horizon_days <= 0:
                raise ValueError(
                    f"Prediction horizon must be positive, got {horizon_days} at row #{idx}"
                )
            ...
            for k, v in row.items():
                if isinstance(v, float) and (math.isnan(v) or math.isinf(v)):
                    raise ValueError(
                        f"Invalid non-finite value '{v}' for key '{k}' at row #{idx} ({full_name})"
                    )
